...
#define RECT_FERRET_VIEW         CGRectMake(10.0f, 30.0f, 300.0f, 380.0f)
...
+ (NSFerret*)ferret
{
    ...
    dispatch_once(&onceToken, ^{
        ferret = [[NSFerret alloc] initWithFrame:[[[UIApplication sharedApplication] keyWindow] bounds]];
    });
    [ferret attachFerretToApplicationWindow: [[UIApplication sharedApplication] keyWindow]];
    return ferret;
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.viewFerret = [[UIView alloc] initWithFrame: RECT_FERRET_VIEW];
        self.viewFerret.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.3];
        ...
        UIButton* buttonClose = [[UIButton alloc] initWithFrame:CGRectMake(240.0f, 10.0f, 50.0f, 50.0f)];
        ...
        UILabel* labelFerretASCII = [[UILabel alloc] initWithFrame: CGRectMake(5.0f, 5.0f, 290.0f, 60.0f)];
        ...
        self.buttonSuperview = [[UIButton alloc] initWithFrame: CGRectMake(10, 250, 280, 40)];
        ...
        [self.viewFerret addSubview: self.buttonSuperview];
        
        self.buttonPrevSibling = [[UIButton alloc] initWithFrame: CGRectMake(10, 292, 80, 40)];
        ...
        [self.viewFerret addSubview: self.buttonPrevSibling];
        
        self.labelNumberOfSiblings = [[UILabel alloc] initWithFrame:CGRectMake(95, 292, 110, 40)];
        ...
        [self.viewFerret addSubview:self.labelNumberOfSiblings];
        
        self.buttonNextSibling = [[UIButton alloc] initWithFrame: CGRectMake(210, 292, 80, 40)];
        ...
        [self.viewFerret addSubview: self.buttonNextSibling];
        
        self.buttonSubviews = [[UIButton alloc] initWithFrame: CGRectMake(10, 334, 280, 40)];
        ...
        [self.viewFerret addSubview: self.buttonSubviews];
...
- (void)attachFerretToApplicationWindow:(UIWindow*)windowApplication
{
    ...
    if (windowApplication) {
        ...
        [self setFrame: self.viewApplication.frame];
        [self.windowApplication addSubview: self];
        [self.viewFerret setFrame: RECT_FERRET_VIEW];
...
- (void)updateFerretView
{
    ...
    self.buttonSuperview.enabled   = (self.viewSelected != self && self.viewSelected != self.windowApplication && self.viewSelected != self.viewApplication);
    self.buttonNextSibling.enabled = ([self indexOfSelectedView] < [self numberOfSiblingViews] - 1);
    self.buttonPrevSibling.enabled = ([self indexOfSelectedView] > 0);
    self.buttonSubviews.enabled    = (self.viewSelected && self.viewSelected.subviews &&
                                      self.viewSelected.subviews.count > 0 && [self.viewSelected.subviews objectAtIndex:0] != nil && self.viewSelected != self);
...
- (CGRect)rectOfViewConvertedToApplicationViewCoordinates:(UIView*)viewToConvert
{
    ...
    while (viewFocus != self.viewApplication && viewFocus != self.windowApplication) {
    ...
    if ([[UIApplication sharedApplication] isStatusBarHidden] == NO) rectConverted.origin.y = rectConverted.origin.y - STATUS_BAR_HEIGHT;
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat X = 260.0f + self.viewFerret.frame.origin.x - RECT_FERRET_VIEW.origin.x;
    CGFloat Y = 135.0f + self.viewFerret.frame.origin.y - RECT_FERRET_VIEW.origin.y;
    CGFloat U =  10.0f;
...
- (void)selectPrevSibling
{
    if (self.viewSelected && self.viewSelected.superview && self.viewSelected.superview.subviews.count > 0) {
        if ([self indexOfSelectedView] > 0) {
            [self setSelectedView: [self.viewSelected.superview.subviews objectAtIndex:[self indexOfSelectedView]-1]];
        }
    }
}
